...
\author{Sarah R. Haile\footnote{Kantonsspital St. Gallen, Bedastrasse 1, 9007 St. Gallen (\texttt{sarah.haile@gmail.com})}}
...
The function also produces the ``event time ratio'' (ETR, $\exp{(-\boldsymbol{\beta}_i/\alpha)} = \exp \boldsymbol{\gamma}_i$), as discussed in \cite{carroll_03}. This ratio quantifies the relative difference in time it takes to achieve the $p$th percentile between two levels of a covariate. The $p$th percentile of the (covariate-adjusted) Weibull distribution occurs at 
